int load(char *filename, PROC *p){
    ...
    //gets to the correct inode
    getblock((iblk + (ino/8)), buf); //mallmans algorithm to get to the right block
    ip = (INODE *)buf + (ino % 8);
    char *addr = (char *)(0x800000 + (p->pid - 1)*0x100000); //PA
    char *addr1 = (char *)(0x1000000 + (p->pid - 1)*0x100000); //PA
    char *addr2 = (char *)(0x1800000 + (p->pid - 1)*0x100000); //PA
    char *addr3 = (char *)(0x2000000 + (p->pid - 1)*0x100000); //PA
    //end
    char *count = addr;
    for (int i=0;i<12;i++){
        //printf("step3\n");
        ...
        if(addr<=(count+=0x100000)){
            getblock(ip->i_block[i], addr);
            addr+=1024;
            printf("addr: %d\t", addr);
            if((addr+1024)>=(count+=0x100000)){
                count=addr1;
            }
        }
        //...could  add other addr points but no need to for right now
        //printf("addr: %x\n", addr);
    }
    printf("%s loaded\n", filename);
    return p->pid;
